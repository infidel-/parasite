...
- **gpt-image cannot emit alpha.** For anything needing transparency (decals, roof details, sprites), generate on a FLAT SOLID grey `#5a5d63` background and register `class:"chroma"` (`chroma:"0x5a5d63"`, `tol:24`) so `make tex` bakes the key → alpha. Windows/facades that must be transparent otherwise stay opaque and the tool warns until hand-edited.
