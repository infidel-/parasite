...
## 3D sewers: `Area3D` seam + tunnels built from the saved cell grid — LANDED

Second area family in 3D (`AREA_SEWERS` + `AREA_HABITAT`, both from `game.SewerAreaGenerator`).

**The seam.** `View` had ~12 city-specific branch points. They moved behind `render.Area3D`
(`scene`/`build`/`tick`/`setTactical`/`bloomThreshold`/`cameraOffsets`/`debris`/`setLampLights`, args
via `Area3DTickOpts`), with `render.CityArea` holding today's city path verbatim and
`render.SewerArea` the new one. Chunking came out of `View` into `render.Chunks`; `Ground`'s
`vtx`/`tri`/`quad` closures into `render.world.MeshBuf`. City verified unchanged: `__check.pass` true,
177 calls / 92.1k tris, crosswalks + lamp cones + occlusion outline all as before.

**The one line that did the work.** `WorldCtx.floorY` is the single ground-height entry point for 40+
call sites (actors, choreo, particles, decals, PathLine, TacticalGrid). A `tiles == null -> 0.0` guard
made the entire actor/FX layer run underground unmodified. Nothing else was touched.

**No seeded generator.** `Sewers.TILE_FLOOR` in the area's already-persisted cells IS the layout, so
`SewerModel.fromArea` reads it directly: works on every existing save, no save-format change, no
`sewerGenSeed`. Variation (wall wear, litter) comes from the per-cell footprint hash, as
`Debris`/`SceneSetup`'s broken lamps already do. Corridor-node lamp placement is ported from
`lighting.SewerAreaLighting`, so 3D lights where 2D lit.

**Chunking was dropped for sewers, deliberately.** The plan had 16-cell chunks; a whole 75x60 level is
5 merged meshes / ~5.4k tris / **30 draw calls**, and chunking would have quadrupled the calls to buy
culling that saves no fill. Nothing fades down here (no ceiling, near-top-down camera), so the
`Occlusion` constraint that blocks every city merge simply does not apply.

**The trap: the fill intensities look absurd and are not.** At the city's ambient 1.6 / hemi 1.3 the
whole level rendered *black* — not dim, black. The art is not the cause: sewer floor measures mean luma
51 against city asphalt's 50. The city frame is carried by a moon, hundreds of lamps and lit windows;
underground the fill IS the frame. A/B'd by driving `renderer.toneMappingExposure` live from CDP: 1.5
black, 6.0 correct, 14.0 washed. That 4x now lives in the lights (ambient 7.5, hemi 3.6) rather than in
exposure, so `vidBrightness` still scales on top. Ambient carries most of it — it is
normal-independent, so vertical walls stay legible against the floor.

**Light counts, and the pre-warm that does not fully close.** Same spot pool (`vidLampLights`),
`MuzzleLights` only, no `FlameLights` — the 5-point permutation `warmup` already caches. `SewerGeom`
on a throwaway `SewerModel.demo()` was added to the boot pre-warm, in **its own `SewerScene`**: a
first attempt parked the geometry in the city warm scene, and it recompiled on entry anyway because
a sewer has no directional light and `NUM_DIR_LIGHTS`/`NUM_DIR_LIGHT_SHADOWS` are in the program key.
The dedicated scene took boot from 64 to 68 programs and warms the tunnel materials at the right
counts. It does **not** get entry to zero: measured `__progs()` 68 boot -> 74 first city -> 81 first
sewer, i.e. **+7 on sewer entry against the city's own +6**. The residue is the shared actor /
particle / decal materials, which the sewer's zero-directional light rig keys differently from the
city warm scene, plus the depth programs for `castShadow` walls (compileAsync does not compile those;
the city gets them from the one real `comp.render()` the warm performs). Same order as the city's own
first-entry cost and hidden under the same enter fade, so it was left there rather than duplicating
the whole actor/particle warm block into a second scene. **Beware measuring this in a session that
already built a sewer** — that reads a flat 73 -> 73 and looks like a clean pass.

**Also landed:** `AreaView.draw` now early-outs on a 3D area. It was being called unconditionally from
`Targeting`/`HUD`/`PlayerArea`/`CultBase` on any `LOCATION_AREA`, painting a full tile pass + LOS
overlay + minimap regen under an opaque WebGL canvas — and underground it would also have dragged in
`AreaLighting` (1663 lines, gated to sewers + lab). The 2D floor-decoration and debris passes were
deleted from `SewerAreaGenerator` (-318 lines); the renderer owns that clutter now
(`render.sewer.Debris`, derived per cell, never persisted). Old saves keep inert tile decorations.

**Not done:** the sewer-specific floor decor (`Sewers.FLOOR_DECOR_META`) is not ported —
`render.decals.Debris` is hardcoded to the `entities` atlas. Adding a per-spot atlas name is the
upgrade path.

## Sewer art restyled to the old 2D tileset — and the sRGB/linear retune trap — LANDED

The first sewer texture set was generated as "dark damp brick" and came out near-black, uniform, and
neutral. Held against the 2D tileset it replaces (`app/img/sewers.png`) it was wrong on four counts:
hue (neutral vs warm rose-brown), value (near-black vs mid-tone), masonry (uniform machine brick ~30
across a tile vs hand-laid blocks of varied size ~8 across), and moss (none vs olive-green in every
joint). All five `sewer/*` sources were regenerated at 1k with the tileset's palette written into the
prompt as hex (`#a8837a`/`#96736a`/`#8d6a60` blocks, `#8f9188` mortar, `#7d8a5e` moss, `#9aa39c`
floor), plus `WALL_H` 6.0 -> 3.0 so the walls read as a cut kerb rather than a shaft.

**The trap: retuning fill against the sRGB byte mean is wrong by ~2.7x.** The fill intensities are
albedo-compensation (see the previous entry — they were driven to 7.5 / 3.6 against near-black art),
so brighter art means they must come down by the albedo ratio. Measured on the sRGB means that ratio
looks like 127.5 / 51 = **2.5x**, giving ambient 3.0. That is wrong: three decodes an sRGB-tagged map
to linear *before* the lambert multiply, and sRGB is strongly concave near black, so the same two
textures are 0.0331 vs 0.2206 in mean linear luminance = **6.66x**. Correct values are ambient 7.5 /
6.66 = **1.13** and hemi **0.54**. Taking the sRGB ratio would have left the level 2.7x too bright.
The general rule: any intensity that compensates for texture brightness is a LINEAR-space ratio.
Script kept at `tools/`-adjacent scratch; recompute with `srgb_to_linear` then Rec.709 luma, then mean.

Notable that the corrected fill (1.13 / 0.54) now sits *below* the city's 1.6 / 1.3 despite the sewer
having no moon — which is the reasoning working, not a contradiction: rendered brightness is albedo x
light, and the tunnel art is far more reflective than city asphalt.

**The albedo ratio is the FLOOR of the fill range, not the answer.** Holding the old rendered
brightness means ambient 1.13, and in-engine that is far too dark — the old art had no masonry or moss
detail to lose and the new art does. Swept live in the habitat at 1x/2x/3x/3.5x/4x of it; **3.5x**
(ambient **3.95**, hemi **1.89**) is where the block courses and joint moss read across the frame
without going flat. Note this lands *below* the city's 1.6/1.3 relative to albedo, which is the
arithmetic working, not a contradiction: brightness is albedo x light.

**Three bugs the in-engine pass caught, none of which the texture work caused:**

1. **`markChannel` flooded open halls with sludge.** "Floor cell whose whole 8-neighbourhood is floor"
   is true of every interior cell of *any* open space, not just a 3-wide tunnel centreline. The 21x14
   habitat is mostly open and its open areas are not generator rooms, so whole rectangles came out
   sludge. Fixed by also requiring the run to be exactly 3 cells wide across at least one axis
   (`!isFloor(col+-2, row)` or `!isFloor(col, row+-2)`). Habitat went from flooded to **14 sludge
   cells against 162 floor (8%)**.
2. **Only shell solid cells were capped.** `shell()` was "orthogonally touches floor", so interior and
   diagonal-only solid cells got no ledge quad — a hole straight through the WALL_H plateau to the
   background. Now every solid cell caps (83 -> 118 cells in the habitat, 2 tris each into an
   already-merged mesh, no new draw call).
3. **Sludge was authored far too dark** — 0.0657 linear against the walkway's 0.2206, so the gutter
   crushed to the background value and read as a pit. Regenerated at **0.1727** (~78% of the
   walkway). Measured in-render: sludge luma 31-34 vs floor 42.5, green-dominant rgb(21,35,32) vs
   blue-dominant rgb(30,45,55).

**A false lead worth recording: wall `castShadow` was NOT the cause and the fix was reverted.** The
dark rectangles look exactly like long hard shadows thrown by 3-unit walls under a 5.6-high bulb, and
that diagnosis survived two eyeball A/Bs of a JPEG ("looks slightly lighter with shadows off").
Sampling the actual pixels killed it: with `shadow.intensity = 0` on all 12 spots the sampled luma was
**identical to the byte** (6.9 / 11.3 / 42.5 before and after). Eyeballing a compressed screenshot is
not a measurement. The general technique that did work, and is worth reaching for first next time:
capture the live renderer/scene through `Object3D.prototype.onBeforeRender` from CDP, then **tint each
merged mesh a distinct flat colour** (floor red / sludge green / ledge blue / wall yellow) and take one
screenshot — that identified the culprit instantly after raycasts had misidentified it twice, because a
ray through a gap happily hits the same-material surface behind it.

## Sewer walls: one base + overlays instead of two tile variants — LANDED

Wall variety was a clean/worn pair picked per solid cell by a footprint hash. Measured over a 200x200
grid at the 38% worn rate: **47.2% of horizontal and 47.4% of vertical cell boundaries changed
variant, median unbroken run ONE 4-unit cell** (theoretical `2p(1-p)` = 47.1%, so the hash is not the
problem — the idea is). Worse, the seams were the only discontinuity present: UVs are world-aligned,
so a same-variant run tiles perfectly and the eye has nothing to look at but the switches. Blending
could not save it either, the two sources having different block layouts — any cross-fade misaligns
mortar against mortar.

**Replaced variety with layers.** One base wall (the worn source promoted, `sewer/wall-worn` folded
into the `sewer/wall` label so no code path moved), plus overlays that cannot seam because the base
under them is continuous: a damp grime band at the foot of every wall, and fake contact shadows where
wall meets floor. New `render.sewer.SewerDetail`; `SewerGeom` keeps the shell.

**Two city systems ported nearly free.** The contact shadows are `RoofShadows` with the hard part
deleted — that class spends most of its length deriving where a parapet exists along a merged
building outline (`coveredEdges`/`exposedSpans`/`cornerBuried`), which underground the cell grid
already answers. And they need NO art: both gradients are canvas-drawn at runtime by
`Textures.makeShadowGradient` / `makeRadialShadowGradient`. Radials go ONLY on diagonal-only solid
neighbours — at an ordinary inside corner the two perpendicular strips already overlap and their
blend *is* the corner darkening, so a radial there double-darkens. The grime band revived
`Textures.loadRampTexture`, which was **dead code** (defined, never called): it bakes the vertical
fade into an opaque tile at load, so the source needs no hand-cut alpha at all, unlike the city grime
it was written for. No `premultipliedAlpha` here — `Buildings` needs it because hand-painted sources
carry junk RGB under near-transparent texels that bleeds cyan/magenta when minified, and a ramp
generated over fully opaque art has no such texels.

**Found and fixed a pre-existing warm-up hole worth more than the feature.** First sewer entry was
compiling +10 programs. Diffing `__progs()` around the entry field-by-field (not by counting) showed
every added lambert/basic key differed from its warmed twin in ONE field: point-light count, 5 vs 0.
`Actors` parks a 5-light `MuzzleLights` pool in every real scene, and `NUM_POINT_LIGHTS` is in every
lit material's program key — the city warm scene carries one for exactly this reason
(`View.hx:317-322`) but the sewer warm scene, added later, never got one. One `MuzzleLights` in the
sewer warm scene took entry **+10 -> +5, and all three lambert keys out of the added set**. The
remaining 5 are the path-line/selection-ring basics and actor-sprite physicals that any first area
entry compiles.

**Also measured:** the contact-shadow material is `FrontSide`, not `RoofShadows`' `DoubleSide` — the
quads lie flat under an always-above camera, and three draws a transparent DoubleSide material as two
single-side passes, so it is worth **2 draw calls (36 -> 34)**. It changed the program count not at
all, so that is a call saving, not a compile saving.

**Verified by A/B, not by eye** (this file's own standing warning): hide the grime + both shadow
instanced meshes, screenshot both states to PNG, diff and amplify 8x. The difference is a continuous
band hugging every wall/floor boundary, tracking the wall outline through steps and corners. The raw
diff also picks up the HUD, whose draw-call readout changes when you hide meshes — crop to the 3D view
before trusting any percentage.

**The decals landed** (`crack-1`, `seepage-1`, `moss-1`, `broken-1`, `sprite` class, alpha hand-cut).
Placement is `WallDecals` minus everything the city needs and a sewer does not — no worn-face test
(every tunnel wall is bare), no door spans, no `userData.b`. The one real change is batching: the city
emits one Mesh PER DECAL because `Occlusion` must fade each with its building, and with no occlusion
pass underground they merge per image, so a whole level costs 4 calls rather than one per decal.
Measured 19 decals over 104 wall faces = **18.3%**, matching `DECAL_PCT = 18`. `seepage` alone carries
`WALLDECAL_TINT` — chalky off-white against night-authored walls, the same reason the city tints
posters but not cracks. Habitat: **34 -> 38 calls**.

**But measure where the pixels actually are before investing more in wall art.** With `WALL_H` at 3.0
and the near-top-down sewer camera, wall FACES are **0.67% of the 3D view** while ledge TOPS are
**14.68% — 22x more** (measured by tinting the two meshes red/green and counting pixels at the default
gameplay camera; it shifts with zoom). So the grime band and all four decals are painting on well
under one percent of the frame: an A/B of decals on/off changed 365 pixels in the whole view, one
visible decal at 33x14. They are correct and they cost almost nothing, but at THIS pitch the surfaces
carrying the level's look are the ledge tops and the floor.

**Read that as "dress the tops too", not as "wall detail was wasted".** The 0.67% is one camera pose,
not the wall's worth: `CAMERA_SEWER.near` is `y 16 / z 12` (~53 deg elevation), where faces are plainly
visible, and the player zooms there. It is the FAR end that flattens them — and tactical pins zoom to
far (`y 40 / z 14`, ~70 deg), so tactical is the view where faces matter LEAST, not most. `WALL_H`
stays 3.0; the follow-up is `SewerGround`, not a taller wall.
