...
class Geom {
  ...
  // dir: 0=+z,1=-z,2=+x,3=-x
  public static var DIRV = CityFaces.DIRV;

  public static inline function imax(a:Float, b:Float):Float return a > b ? a : b;

  // Street = the cells directly outside are road/walkway. CityFaces owns the rule so
  // CityGen's blank-box drop and the render's window pass can never disagree
  public static inline function faceIsStreet(b:Building, dir:Int):Bool
    return CityFaces.faceIsStreet(WorldCtx.tiles, b, dir);
  ...
  // tile-index runs of face `dir` that look onto open space and so earn windows (see
  // CityFaces.openWinRuns for the rule). inclusive {lo,hi} along x for dir 0/1, z for dir 2/3
  public static inline function openWinRuns(b:Building, dir:Int, allowPartial:Bool):Array<{lo:Int, hi:Int}>
    return CityFaces.openWinRuns(WorldCtx.tiles, b, dir, allowPartial);
