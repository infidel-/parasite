...
// the grown props' IDLE MOTION, folded into the materials they already draw with. two terms, both
// pure vertex shader, both driven by one shared clock:
...
//            (which keys on normal.y). with RenderConfig.PROP_LIGHT.pool at 0 an organ standing away
//            from a bracket has only the hemisphere term, so expect this half to be faint until a
//            prop light is back on — see docs/3d-changes.md
//
...
//
// WHICH props move is not decided here — it is the `anim` column of render.world.ObjModels.MODELS,
// and a null row is simply never patched (so it also keeps the shared unpatched program)
class PropShader
{
...
// frames — the same reason SewerMask.patchMesh exists and is called per frame rather than at build
  public static function patchMesh(o:Object3D, a:PropAnim):Void
    {
      if (a == null ||
...
// through one path. Typing it would take a Material supertype in the extern, which is its own change
  public static function patch(mat:Dynamic, geo:BufferGeometry, a:PropAnim):Void
    {
      ...
      var base = geo.boundingBox.min.y;
      var span = geo.boundingBox.max.y - base;
      if (span <= 0)
        return;
      ...
      var u = {
        ...
        strand: { value: a.strand / span },
        sheen: { value: a.sheen },
        sheenRate: { value: a.sheenRate },
      };
      ...
      var phase =
        ...
        '    propPh = instanceMatrix[ 3 ].x * 0.37 + instanceMatrix[ 3 ].z * 0.29;\n' +
        '  #endif\n' +
        '  propPh += ( position.x + position.z ) * propStrand;\n';
      ...
      // and unlit forms above would collide with each other. CHAINED, because the props' solid and
      // ghost materials already carry SewerMask's hook and its key
