...
//
// WHICH props move is not decided here — it is the `anim` and `pulse` columns of
// render.world.ObjModels.MODELS. the two are independent, either may be null, and a row with both
// null is simply never patched (so it also keeps the shared unpatched program)
class PropShader
{
...
// frames — the same reason SewerMask.patchMesh exists and is called per frame rather than at build
  public static function patchMesh(o:Object3D, a:PropAnim, p:PropPulse):Void
    {
      if ((a == null && p == null) ||
      ...
      if (o != null &&
          o.material != null &&
          o.geometry != null)
        patch(o.material, o.geometry, a, p);
    }
...
// through one path. Typing it would take a Material supertype in the extern, which is its own change
  public static function patch(mat:Dynamic, geo:BufferGeometry, a:PropAnim, p:PropPulse):Void
    {
      ...
      mat.customProgramCacheKey = function()
        return (prevKey != null ? Std.string(Reflect.callMethod(mat, prevKey, [])) : '') +
          'propAnim' + (hasA ? 'S' : '') + (hasP ? 'U' : '') + (lit ? 'L' : 'P');
