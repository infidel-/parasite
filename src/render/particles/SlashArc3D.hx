...
// a glowing procedural attack-arc sprite spawned at a melee strike, keyed by the weapon's
// _AttackEffect. the shape is a runtime-rasterized white SVG (crescent / overhead arc / fist / claw
// / bolt) tinted to the weapon's color via emissive, grown + faded over its life. per kind it lies
// flat on the ground or stands upright, and may slide from the attacker to the target. purely visual
class SlashArc3D extends Particle3D {
  ...
  public function new(effect:String, ax:Float, ay:Float, az:Float, bx:Float, by:Float, bz:Float)
    {
      ...
      // per-kind tuning from the render config arc table (typed lookup by the _AttackEffect enum)
      var c = RenderConfig.ARC.kinds;
      var k = switch (effect)
        {
      ...
      this.life = RenderConfig.ARC.lifeMult;
...
// faded by progress, laid flat + swing-aligned or upright + roll-swept
  override public function draw(p:Paint3D):Void
    {
      var tex = p.sprites.svgTex(shape + ':' + RenderConfig.ARC.px, svg, RenderConfig.ARC.px);
      if (tex == null)
        return;
      var u = t / life;
      ...
      p.sprites.paint(cx, cy, cz, tex, 1 - u, scale * (0.6 + 0.5 * u), flat, orient,
        Sprites.ORD_ACTOR + 1, color, RenderConfig.ARC.emissiveInt);
    }
