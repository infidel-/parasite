  ...
  function init()
    {
      ...
      mods.ModLoader.load(game).then(function(_) {
        ...
        // pre-warm the 3D shaders off the menu idle so the first area entry doesn't stall ~2s under
        // black compiling them on demand (see render.View.warmup). the 3D canvas is hidden
        // (#view display:none) so this is invisible; delayed a beat so the menu paints first
        Browser.window.setTimeout(function() game.scene.view3d.warmup(), 300);
        return null;
      });
    }
