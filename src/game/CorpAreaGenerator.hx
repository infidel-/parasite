...
// generate a hangar of a given size at given coordinates
  function generateHangar(state: _CorpState,
...
// fill rooms with content
  function generateRoomContents(state: _CorpState)
    {
      for (room in state.rooms)
        {
...
// make this room a chem lab
  function roomChemistryLab(state: _CorpState, room: _Room)
    {
      var area = state.area;
      var cells = area.getCells();
...
// add decoration from a list
  function addDecoration(state: _CorpState, x: Int, y: Int, infos: Array<_TileRow>)
    {
