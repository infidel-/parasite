...
| `fx`            | `ModFx`          | named fx registry + RAF/canvas/overlay primitives. Register your effects by id (must start with `mod-<modID>-`), fire them with `parasite.fx.play(id, params)`, compose using `tick` (RAF scheduler), `viewport()` (whichever canvas is live — use this for camera shake), `canvas()` (the 2D `#canvas` specifically), `overlay()` (engine-owned reusable fullscreen div). See [05-monkey-patching.md](05-monkey-patching.md#fx-system). |
