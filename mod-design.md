...
- ~~Runtime-gated debug build~~ → debug is runtime-gated by a `.debug` sentinel file in the game install/current working directory, resolved once by main at `App.ready`. Main exposes the result through `host:boot` / `HostBridge.isDebug`; `Main.init()` copies it into `Const.isDebug` before `Game` / `Config` construction. Debug menu/devtools, console cheat branches, HUD/tooltip debug overlays, profile timers, and `host:debug:*` image/text dump IPC are all gated on this flag. There is no separate `#if mydebug` build split anymore; absent `.debug`, the release behavior is the default and debug IPC handlers refuse writes.
